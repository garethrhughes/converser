...
**Project:** Converser — Ingest conversations from Google Drive, parse with custom AI agents (Bedrock), categorise and report on themes by People/Clients
**Skillset profile:** `typescript` (rules: `RULES.md` + `rules/typescript.md`)

**Backend:** NestJS 11 / TypeScript strict / PostgreSQL 16 + TypeORM
**Frontend:** Next.js 16 (App Router) + React 19 / Tailwind CSS v4 / Zustand
**Auth:** Google OAuth 2.0 (OIDC) — JWT 15min access + 7-day refresh rotation
**Validation:** class-validator + class-transformer
**Logging:** pino → CloudWatch Logs
**Testing:** Jest + Supertest / Vitest + React Testing Library

**Infra:** Terraform on AWS; state in S3 + DynamoDB; secrets in SSM Parameter Store; CI/CD via GitHub Actions
**Local dev:** Docker Compose — PostgreSQL 16

**Compliance:** ISO 27001:2022
**Data classes:** public / internal / confidential / pii
**Encryption:** at rest AES-256 (KMS for confidential/pii) / in transit TLS 1.2+

**Repo structure:** `backend/`, `frontend/`, `infra/`, `docs/`, `scripts/`
**Module structure:** One NestJS module per feature domain (auth, conversations, people, agents, themes, reports). Frontend uses App Router pages with shared components, stores, and typed API lib.

**Key rules:** Standard rules from `RULES.md`. Project-specific additions:
- _(none)_

**External integrations:** Google Drive API (ingestion), AWS Bedrock (AI agents), Google OAuth 2.0 (auth)
**Key entities:** Conversation (confidential), Person/Client (confidential), Agent (internal), AgentInstruction (internal), Theme (internal), Category (internal), Report (internal)
**Known gotchas:** Drive API rate limits, Bedrock token/context limits, partial ingestion failures, timezone handling, PII in conversation content
**Jira:** none
